...
export default function ProductForm({
  ...
  async function saveProduct(ev) {
    // I am using preventDefault because the default behavior of the form is use a get request and paste everything to the url and in this case I don't want to do that
    ev.preventDefault();
    const data = { title, description, price, images, category, properties: productProperties };
    if (_id) {
      // update
      ...
      // I used yarn import axios for the api instead of the normal fetch that I do
      await axios.post("/api/products", data);
      // This next part redirects to the products page.  It is initially set to false as we defined earlier, but we then want it to be true
    }
    setGoToProducts(true);
  }
  ...
  return (
    <form onSubmit={saveProduct}>
      ...
      <div className="mb-2 flex flex-wrap gap-1">
        ...
        <label className="w-24 h-24 cursor-pointer text-center flex flex-col items-center justify-center text-sm gap-1 text-primary rounded-sm bg-white shadow-sm border border-primary">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
            <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5 0h12m-12-1.5V6.75A2.25 2.25 0 015.25 4.5h13.5A2.25 2.25 0 0121 6.75v8.25M3 16.5l3.6-3.6a1.5 1.5 0 012.1 0l2.4 2.4M21 16.5l-3.6-3.6a1.5 1.5 0 00-2.1 0l-2.4 2.4" />
          </svg>
          <span>Upload</span>
          <input type="file" accept=".jpg,.jpeg,.png" multiple onChange={uploadImages} className="hidden" />
        </label>
      </div>
      <label>Description</label>
      <textarea
